...
-- 여직원과 남직원은 각각 몇 명이나 있나요?
	select count(case when gender='f' then 1 end) as '여직원' ,
		   count(case when gender='m' then 1 end) as '남직원'
	from employees;
-- 문제4.
-- 현재 근무하고 있는 직원 수는 몇명 입니까? (salaries 테이블을 사용합니다.)
-- select count(emp_no) as "직원 수" from salaries;
...
-- select * from salaries where salary >= 120000 and to_date Like '9999%' limit 500;
	select count(emp_no) 
	from salaries 
	where salary >= 120000 
